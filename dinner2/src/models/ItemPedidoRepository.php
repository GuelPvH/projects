...
class ItemPedidoRepository{
    ...
    public function adicionarProduto(PedidosRepository $pedido, string $produto, ?int $quantidade){
        ...
        // Fazer tratamento de erro para caso não tenha o produto
        $stmt->bindValue(':nome_produto', strtolower($produto));
        $stmt->bindValue(':nome_produto', $quantidade);
        $stmt->execute();
    }

    public function removerProduto(PedidosRepository $pedido, string $produto, ?int $quantidade){
    ...
    public function listarItensPedido(PedidosRepository $pedido){
